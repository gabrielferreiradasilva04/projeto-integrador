<template>
    <v-dialog v-model="dialog" scrollable persistent max-width="60%" transition="dialog-transition" class="h-auto w-auto">
        <v-card class="rounded-xl pa-2 elevation-0">
            <v-card-title>
                Informe um valor para retirada a partir de R$20,00
            </v-card-title>
            <v-card class="pa-2" variant="text">
                <v-form>
                    <v-text-field loading name="amount" label="Valor a retirar" placeholder="100.00" id="amount"
                        v-model="this.amount" variant="solo-filled"></v-text-field>
                    <v-text-field
                        name="pix"
                        label="Informe sua chave PIX para retirada"
                        id="pix"
                        variant="solo-filled"
                    ></v-text-field>
                </v-form>
                <v-card-actions>
                    <v-btn @click="this.removeBalance" variant="text" color="success">Finalizar</v-btn>
                    <v-btn color="warning"
                        @click="this.$emit('closeRemoveBalance')"><v-icon>mdi-keyboard-return</v-icon></v-btn>
                </v-card-actions>
            </v-card>
...
export default {
    ...
    methods: {
        async removeBalance() {
            await fetch('http://localhost:8081/movement/removeBalance?walletId=' + this.store.state.wallet.id + '&amount=' + this.amount, {
                method: 'POST',
                headers: { 'Content-type': 'application/json' },
            }).then(res => {
                if (res.status === 200) {
                    this.infoMessage = 'Retirada realizada com sucesso!'
                    this.showMessage = true;
                    this.store.methods.getCurrentUser();
                    setTimeout(() => {
                        this.$emit('closeRemoveBalance')
                    }, 1000);
                }
                if (res.status === 404) {
                    res.json().then(data => {
